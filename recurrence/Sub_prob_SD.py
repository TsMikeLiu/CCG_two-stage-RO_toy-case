...
g = SDSP.addVars(3, lb=0, ub=1, vtype=GRB.CONTINUOUS, name='g')
...
SDSP_Cons3 = SDSP.addConstr(quicksum(g[i] for i in range(3)) <= 1.8, name='SDSP_cons3')
SDSP_Cons4 = SDSP.addConstr(quicksum(g[i] for i in range(2)) <= 1.2, name='SDSP_cons4')

# obj = quicksum(-p[i]*z[i].x for i in range(3)) + quicksum(t[j]*d[j] for j in range(3))
d = [dl[i] + du[i] * g[i] for i in range(3)]
