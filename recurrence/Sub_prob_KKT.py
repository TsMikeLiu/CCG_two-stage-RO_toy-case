...
g = SP.addVars(3, lb=0, ub=1, vtype=GRB.CONTINUOUS, name='g')
d = SP.addVars(3, lb=0, vtype=GRB.CONTINUOUS, name='d')
# d = [206+40*g[0], 274+40*g[1], 220+40*g[2]]

# dual
pi = SP.addVars(3, lb=0, vtype=GRB.CONTINUOUS, name='pi')
theta = SP.addVars(3, lb=0, vtype=GRB.CONTINUOUS, name='theta')
...
# dual
SP_C3 = SP.addConstrs(theta[j]-pi[i] <= C[i][j] for i in range(3) for j in range(3))
...
SP_C12 = SP.addConstrs(d[i] == dl[i] + du[i]*g[i] for i in range(3))
