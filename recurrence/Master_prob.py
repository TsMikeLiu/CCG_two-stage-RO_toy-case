...
MP_Cons_3 = MP.addConstrs((eta >= quicksum(x_ij[i,j]*C[i][j] for i in range(3)) for j in range(3)), name='MP_Cons_3')
MP_Cons_4 = MP.addConstrs((quicksum(x_ij[i,j] for j in range(3)) <= z[i] for i in range(3)), name='MP_Cons_4')
MP_Cons_5 = MP.addConstrs((quicksum(x_ij[i,j] for i in range(3)) >= d[j] for j in range(3)), name='MP_Cons_5')

Cons_3 = MP.addConstr((quicksum(g[i] for i in range(3)) <= 1.8), name='MP_Cons_3')
Cons_4 = MP.addConstr((quicksum(g[i] for i in range(2)) <= 1.2), name='MP_Cons_4')
Cons_5 = MP.addConstrs(g[i]*du[i] + dl[i] == d[i] for i in range(3))
